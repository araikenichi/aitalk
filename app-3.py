...
if st.session_state["messages"]:
    ...
    for message in reversed(messages):
        ...
        st.markdown(
            f"<div style='display: flex; margin-bottom: 20px; justify-content: {message_align}; align-items: center;'><span style='margin-right: 10px; text-align: {text_align}; transform: rotate(180deg);'>{message['content']}</span> {speaker_icon}</div>",
            unsafe_allow_html=True,
        )
