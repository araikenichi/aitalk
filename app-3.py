...
if st.session_state["messages"]:
    ...
    for message in reversed(messages):
        if message["role"] == "user":
            ...
            content_style = "background-color: #008D16; color: black; padding: 10px; border-radius: 10px;"
            content_order = f"<span style='{content_style}'>{message['content']}</span>"
        else:
            message_align = "flex-start"
            content_style = "background-color: #666966; color: white; padding: 10px; border-radius: 10px;"
            content_order = f"<span style='{content_style}'>{message['content']}</span>"
